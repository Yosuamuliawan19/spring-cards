...
export default class DashBoardPage extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dropDown: -1,
      searchTerm: '',
      toast: { color: 'white', text: 'Has been deleted', show: false },
      MouseX: 0,
      MouseY: 0,
      rowView: false
    }
      ...
      <div className="dashboard-container" onMouseMove={this._onMouseMove.bind(this)}>
        <div className="dashboard-nav">
          <div className="dashboard-nav-left">
            ...
            <input
              class="shadow-md my-auto ml-8 py-3 px-4 text-gray-900  rounded-lg focus:ring-2 focus:ring-blue-600 ..."
          ...
          <button className="btn-1 btn-create" onClick={(_) => (window.location.href = '/create')}>
            Create new shelf
          </button>
        </div>
        ...
        <div className={this.state.toast.show ? 'shadow-lg toast-hide toast-show ' : 'shadow-lg toast-hide'}>{this.state.toast.text}</div>

        <div className="folder-section grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 items-stretch gap-4">
          {this.state.shelves
            .filter((data) => {
              let reg = new RegExp(`\\b${this.state.searchTerm}`, 'i')
            ...
            .map((data, id) => {
              ...
              return (
                <div
                  key={id}
                  className={
                    this.state.rowView ? 'swing-in-top-fwd folder-container folder-container-row' : 'swing-in-top-fwd folder-container'
                  }>
                  <div
                    onClick={(_) => this.goToView()}
                    style={{ background: `url(${data.img})` }}
                    className={this.state.rowView ? 'folder folder-row shadow-lg' : 'folder shadow-lg'}></div>
                  <div className={this.state.rowView ? 'folder-name folder-name-row' : 'folder-name'}>
                    <div onClick={(_) => this.goToView()}>{data.name.substr(0, 20)}</div>
